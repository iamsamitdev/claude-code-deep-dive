...
แนวคิดหลักของวันนี้คือ "**อย่าทำงานคนเดียว — สร้างทีม AI ให้ทำแทน**" ด้วยเครื่องมือห้าชิ้นที่ทรงพลังที่สุดใน Claude Code:

1. **Custom Slash Commands** - สูตร prompt สำเร็จรูปของทีมใน `.claude/commands/` (ย้ายมาจากวันที่ 2 ให้ตรงกับ Phase 3 ใน spec)
2. **Sub-agents** — ผู้ช่วย AI เฉพาะทางที่มี context แยกของตัวเอง
3. **MCP (Model Context Protocol)** — โปรโตคอลเชื่อมต่อ Claude กับโลกภายนอก เช่น ฐานข้อมูล, GitHub
4. **Hooks** — ระบบ automation ที่รันสคริปต์อัตโนมัติเมื่อ Claude ทำงาน
5. **Skills** — คู่มือ/ความรู้เฉพาะทางที่ Claude หยิบมาใช้เองอัตโนมัติเมื่อ task ตรงกับ description
...
- งานที่ Claude สรุปตรงกับ Phase 3 ใน spec ไหม (Custom Slash Commands ที่ย้ายมาจาก Phase 2, Sub-agents code-reviewer/test-writer/security-auditor, MCP PostgreSQL+GitHub, Hooks)
...
## Module 3.0: Custom Slash Commands (ย้ายมาจากวันที่ 2)
...
```
/create-crud Category
/add-feature stock-alert
/review-code app/actions/products.ts
```
...
> **Key Concept:** Custom Slash Commands คือ "สูตรสำเร็จ" ของทีม - เขียนครั้งเดียว ทุกคนในทีมใช้ได้เหมือนกัน ไม่ต้องพิมพ์ prompt ยาว ๆ ซ้ำ ๆ และ convention ของโปรเจกต์ถูกฝังไว้ในคำสั่งแล้ว

### ทำไม Custom Commands ถึงสำคัญ?

| ปัญหาเดิม | แก้ด้วย Custom Commands |
|---|---|
| พิมพ์ prompt ยาวซ้ำ ๆ ทุกครั้ง | พิมพ์ `/create-crud Category` ครั้งเดียว |
...
### 🛠️ ขั้นตอนที่ 0.1: สร้างโฟลเดอร์ commands และ Command `/create-crud`

**💬 Prompt ที่ใช้สั่ง Claude Code:**
```
สร้างโฟลเดอร์ .claude/commands/ และไฟล์ .claude/commands/create-crud.md
เนื้อหาคือ Custom Slash Command สำหรับ /create-crud ที่:
- รับ $ARGUMENTS เป็นชื่อ Prisma model
- อ่าน @prisma/schema.prisma และ @CLAUDE.md ก่อนสร้างโค้ด
- สร้างตาม convention ของโปรเจกต์นี้:
  - Server Actions ใน app/actions/[model_lower].ts (create/update/delete แบบ soft-delete
    ถ้า model มี deletedAt) คืนค่า ActionResult จาก lib/types.ts
  - Zod schema เพิ่มใน lib/validations.ts
  - read query เพิ่มใน lib/queries.ts (filter deletedAt: null ถ้ามี)
  - หน้า list ที่ app/[model_lower]/page.tsx (RSC + force-dynamic + AppShell)
    และฟอร์มแบบ dialog ใน components/[model_lower]/
- กฎ: ห้ามมี semicolon, 'use server', revalidatePath หลัง mutation,
  ใช้ design token ห้าม hardcode สี hex
```

**🤖 Claude Code จะทำอะไร:** สร้างโฟลเดอร์และไฟล์ `.claude/commands/create-crud.md` พร้อม frontmatter และ prompt template ที่ฝัง convention จริงของ stock-app

**✅ Checkpoint ตรวจสอบ:**
- โฟลเดอร์ `.claude/commands/` ถูกสร้างขึ้น
- ไฟล์ `create-crud.md` มี `---` frontmatter และ `description:`
- มี `$ARGUMENTS` ปรากฏในเนื้อหา
- convention ตรงกับโปรเจกต์จริง: `app/actions/`, `ActionResult`, soft-delete, ห้าม semicolon

**📄 ไฟล์อ้างอิง - ผลลัพธ์ที่ควรได้ (ไว้เทียบ ไม่ใช่พิมพ์เอง):**
```markdown
---
description: สร้าง CRUD ครบชุดสำหรับ Prisma model ที่ระบุ ตาม convention ของ stock-app
---

สร้าง CRUD ครบชุดสำหรับ model ชื่อ "$ARGUMENTS" ของโปรเจกต์ stock-app

## ข้อกำหนด

1. อ่าน @prisma/schema.prisma และ @CLAUDE.md ก่อนเสมอ
2. โครงสร้างไฟล์ตาม convention:
   - `app/actions/$ARGUMENTS_lower.ts` - Server Actions (create/update/delete)
   - `lib/validations.ts` - เพิ่ม zod schema ของ model นี้
   - `lib/queries.ts` - เพิ่ม read query (filter deletedAt: null ถ้ามี)
   - `app/$ARGUMENTS_lower/page.tsx` - หน้า list (RSC + force-dynamic + AppShell)
   - `components/$ARGUMENTS_lower/` - client component ตาราง + dialog ฟอร์ม

3. กฎที่ต้องปฏิบัติตาม:
   - ห้ามใส่ semicolon (;) ท้ายบรรทัด
   - 'use server' ใน actions ทุกไฟล์ และคืนค่า ActionResult จาก lib/types.ts
   - validate ด้วย zod ทุก input / แสดง fieldErrors รายช่อง
   - delete เป็น soft-delete ถ้า model มี deletedAt
   - revalidatePath ทุกหน้าที่เกี่ยวหลัง mutation + sonner toast ฝั่ง client
   - UI ภาษาไทย ใช้ design token ของธีม Genius Stock ห้าม hardcode hex
```

### 🛠️ ขั้นตอนที่ 0.2: สร้าง Command `/add-feature` และ `/review-code`

**💬 Prompt ที่ใช้สั่ง Claude Code:**
```
สร้างไฟล์ .claude/commands/add-feature.md สำหรับ command /add-feature ที่:
- รับ $ARGUMENTS เป็นชื่อฟีเจอร์ที่ต้องการ
- วิเคราะห์ @prisma/schema.prisma @CLAUDE.md และ @docs/spec.md ก่อนว่ากระทบ model ไหน
- แสดง plan เป็น checklist รอการยืนยันก่อนลงมือ และเตือนให้อัปเดต docs/spec.md
- กฎ: ห้าม semicolon, ใช้ TypeScript strict, Server Actions + ActionResult
...
- ตรวจ Correctness, Convention (no semicolon, ActionResult, deletedAt filter),
...
- ทดสอบใช้จริง: `/review-code app/actions/stock.ts` ควรได้รายงานครบ 5 หัวข้อ

### แนวคิด: Commands เป็นสินทรัพย์ของทีม

`.claude/commands/` ควร commit เข้า Git - ทุกคนในทีม clone repo มาแล้วใช้ command เดียวกันได้ทันที ผลลัพธ์สม่ำเสมอตรงตาม convention และ onboard คนใหม่เร็วขึ้นมาก เมื่อทำเสร็จอย่าลืมติ๊ก checkbox งาน Custom Slash Commands ใน `docs/spec.md` (Phase 3)
...
```
---
name: ชื่อ agent (ตัวพิมพ์เล็ก + ขีดกลาง ไม่มีช่องว่าง)
...
> **หมายเหตุ format:** เอกสารปัจจุบันกำหนด `tools` เป็น string คั่นด้วย comma (เช่น `tools: Read, Grep, Glob`) และ field บังคับมีแค่ `name` กับ `description` - ถ้าไม่ระบุ `tools` agent จะได้รับทุก tool, ไม่ระบุ `model` จะเป็น `inherit`

**ตัวเลือก `model`** (ใช้ alias จะได้ไม่ต้องแก้ไฟล์ทุกครั้งที่มีรุ่นใหม่):

| ค่า | ใช้เมื่อ |
|---|---|
| `opus` | งานซับซ้อน ต้องการคุณภาพสูงสุด |
| `sonnet` | งานทั่วไป สมดุลระหว่างคุณภาพและความเร็ว |
| `haiku` | งานง่าย ต้องการความเร็ว ประหยัด token |
| `inherit` (default) | ใช้ model เดียวกับ main agent |
| model ID เต็ม | เช่น `claude-opus-4-8` เมื่อต้องล็อกรุ่นเจาะจง |
...
โดยมี:
- frontmatter: name=code-reviewer, model=sonnet, tools: Read, Grep, Glob
...
- พิมพ์ `@` ใน prompt แล้วเห็น `code-reviewer` ใน typeahead (ถ้าไม่เห็น ให้ restart Claude Code ก่อน - ดูหัวข้อ 3.2.4)
...
โดยมี:
- frontmatter: name=test-writer, model=sonnet,
...
โดยมี:
- frontmatter: name=security-auditor, model=opus, tools: Read, Grep, Glob
...
**🤖 Claude Code จะทำอะไร:** สร้างไฟล์ `.claude/agents/security-auditor.md` ตั้ง `model: opus` (ต่างจาก agent อื่น) เพื่อให้ได้ผลการ audit ที่ละเอียดที่สุด

**✅ Checkpoint ตรวจสอบ:**
- `model` ต้องเป็น `opus` (ไม่ใช่ sonnet) — จุดที่ AI มักตั้งค่าผิดโดยใช้ model เดิมกับ agent ก่อนหน้า
...
### 3.2.4 จัดการและตรวจสอบ Sub-agents (เวอร์ชันปัจจุบัน)

> **⚠️ อัพเดทสำคัญ:** ตั้งแต่ Claude Code **v2.1.198** เป็นต้นมา คำสั่ง `/agents` **ไม่เปิด interactive wizard แล้ว** - พิมพ์ `/agents` จะได้เพียงข้อความแจ้งว่า wizard ถูกถอดออก และแนะนำให้จัดการ agent ผ่าน 2 วิธีแทน:
>
> 1. **สั่ง Claude โดยตรง** เช่น "create a code-reviewer subagent that ..." หรือ "ช่วยแก้ไข agent test-writer ให้ ..."
> 2. **แก้ไขไฟล์เอง** ที่ `.claude/agents/` (ระดับโปรเจกต์) หรือ `~/.claude/agents/` (ทุกโปรเจกต์)
>
> โครงสร้างไฟล์ agent, frontmatter และตำแหน่งโฟลเดอร์ **ยังเหมือนเดิมทุกอย่าง** - สิ่งที่หายไปคือ wizard ในเทอร์มินัลเท่านั้น (อ้างอิง: https://code.claude.com/docs/en/sub-agents)

#### การ Restart Claude Code หลังสร้าง agent ครั้งแรก

โดยปกติ Claude Code จะเฝ้าดู (watch) โฟลเดอร์ `.claude/agents/` และ `~/.claude/agents/` อยู่แล้ว - เมื่อเพิ่มหรือแก้ไขไฟล์ agent ระบบจะตรวจพบภายในไม่กี่วินาทีโดย **ไม่ต้อง restart**

**แต่มีข้อยกเว้นสำคัญ:** watcher ครอบคลุมเฉพาะโฟลเดอร์ที่ "มีอยู่แล้วตอนเริ่ม session" ดังนั้นกรณีของเราที่เพิ่งสร้างโฟลเดอร์ `.claude/agents/` เป็นครั้งแรกใน session นี้ ต้อง **restart Claude Code ก่อน** agent ทั้ง 3 จึงจะถูกโหลด:

```
1. ออกจาก Claude Code ด้วย /exit (หรือกด Ctrl+C สองครั้ง)
2. เปิดใหม่ด้วยคำสั่ง claude ในโฟลเดอร์ stock-app เดิม
```

หลังจากนี้ หากแก้ไขหรือเพิ่มไฟล์ agent ในโฟลเดอร์เดิม ไม่ต้อง restart อีก - ระบบ hot-reload ให้อัตโนมัติ

#### วิธีตรวจสอบว่า agent โหลดครบ

เมื่อไม่มี wizard ให้เรียกดูแล้ว ใช้วิธีเหล่านี้แทน:

**วิธีที่ 1 — พิมพ์ `@` ใน prompt:** agent ที่โหลดแล้วจะปรากฏใน typeahead ให้เลือก (เรียกตรงได้ด้วย `@agent-code-reviewer`)

**วิธีที่ 2 — ถาม Claude โดยตรง:**
```
มี sub-agents อะไรบ้างในโปรเจกต์นี้ อ่านจาก .claude/agents/ แล้วสรุปให้หน่อย
```
...
**วิธีที่ 3 — ตรวจไฟล์เองใน terminal:**
```bash
ls .claude/agents/
```

> **Key Concept:** Claude Code อ่าน `description` ใน frontmatter เพื่อตัดสินใจว่าจะเรียก sub-agent ตัวไหน เมื่อ main agent ได้รับ prompt ที่ตรงกับคำอธิบาย มันจะ delegate งานไปให้อัตโนมัติ หรือคุณสั่งตรงได้โดยระบุชื่อ agent (หรือใช้ `@agent-<ชื่อ>`)
...
```
ช่วยตรวจสอบไฟล์ app/actions/products.ts ก่อนที่ฉันจะ merge
```
...
```
ใช้ test-writer เขียน test สำหรับ lib/product-status.ts
```

**วิธีที่ 3 — สั่งงานหลาย agent พร้อมกัน (Parallel):**
```
ช่วยทำทั้งสองอย่างพร้อมกัน:
1. code-reviewer: ตรวจ app/actions/products.ts
2. test-writer: เขียน test สำหรับ lib/product-status.ts
```
...
1. code-reviewer: ตรวจสอบ app/actions/stock.ts และ components/stock/stock-form.tsx
2. test-writer: เขียน test ครอบคลุม business logic Stock Out (ห้ามเบิกเกิน quantity + กัน race)
3. security-auditor: สแกน app/actions/ ทั้งโฟลเดอร์
```
...
|---|---|
| "ตรวจโค้ดหน่อย" | "code-reviewer: ตรวจ `app/actions/` ทั้งโฟลเดอร์ โดยเน้นเรื่อง N+1 query และ error handling" |
...
| "ดูความปลอดภัย" | "security-auditor: สแกน Server Actions ทั้งหมดใน `app/actions/` เน้น input validation และการกันเบิกเกิน" |
...
**✅ Checkpoint ตรวจสอบ:** พิมพ์ `@` ใน prompt แล้วต้องเห็น agent ทั้ง 3 ตัวใน typeahead (จำไว้ว่า `/agents` wizard ถูกถอดออกแล้ว - ถ้าไม่เห็น ให้ restart Claude Code เพราะเพิ่งสร้างโฟลเดอร์ `.claude/agents/` ครั้งแรกใน session)
...
เน้นที่:
1. app/actions/ — ตรวจ input validation, การกันเบิกเกิน และ error handling
2. lib/queries.ts — ตรวจว่าทุก query กรอง deletedAt: null ครบ (ข้อมูลรั่วจาก soft-delete)
3. ไฟล์ .env และ config — ตรวจ secrets exposure
รายงานพร้อม severity level และ Security Score
```

**🤖 Claude Code จะทำอะไร:** เรียก `security-auditor` sub-agent (model: opus) ให้สแกนโปรเจกต์และออก report พร้อม Security Score
...
- ✅ สร้าง Custom Slash Commands ใน `.claude/commands/` พร้อม `$ARGUMENTS` (`/create-crud`, `/add-feature`, `/review-code`) - ย้ายมาจากวันที่ 2 ให้ตรง Phase 3
...
- ✅ ตรวจสอบ agents ผ่าน `@` typeahead และการอ่านไฟล์ `.claude/agents/` (แทน `/agents` wizard ที่ถูกถอดออกตั้งแต่ v2.1.198)
...
- [Claude Code Sub-agents Documentation](https://code.claude.com/docs/en/sub-agents) — เอกสารทางการ sub-agents (เวอร์ชันปัจจุบัน)
- [Claude Code Slash Commands](https://code.claude.com/docs/en/slash-commands) — เอกสาร Custom Slash Commands
