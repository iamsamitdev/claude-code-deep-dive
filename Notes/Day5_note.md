...
วันที่ 5 คือวันสุดท้ายและหัวใจสำคัญที่สุดของหลักสูตร วันที่เราพา **StockApp** ออกจากเครื่อง dev ไปยืนบน internet จริง ผู้ใช้ทั่วโลกเปิดได้ผ่าน `https://stock.aibisec.com`
...
| **โดเมน** | stock.aibisec.com ชี้ A record มาที่ IP นี้ |
...
2. app: image จาก ghcr.io/YOUR_GITHUB_USERNAME/stock-app:latest
   ...
   BETTER_AUTH_URL=https://stock.aibisec.com, NEXT_PUBLIC_APP_URL=https://stock.aibisec.com, NODE_ENV=production
...
- `BETTER_AUTH_URL` และ `NEXT_PUBLIC_APP_URL` ต้องเป็น `https://stock.aibisec.com` ห้ามใช้ `http://` ; `BETTER_AUTH_SECRET` ต้องเป็น random ≥ 32 ตัว (`openssl rand -base64 32`)
...
### 🛠️ ขั้นตอนที่ 7: ร่าง Nginx Server Block Config สำหรับ stock.aibisec.com

**💬 Prompt ที่ใช้สั่ง Claude Code:**
```
ช่วยร่าง Nginx server block config สำหรับ stock.aibisec.com
...
- หลัง reload ทดสอบ `curl -I http://stock.aibisec.com` ได้ HTTP 200 หรือ 301
...
### 📖 ความรู้เสริม: DNS Record Types แต่ละชนิดคืออะไร

ในหน้า Manage DNS ของผู้ให้บริการโดเมน (เช่น name.com) ช่อง **TYPE** จะมีชนิดของ record ให้เลือกหลายแบบ แต่ละชนิดทำหน้าที่ต่างกัน ต่อไปนี้คือความหมายของแต่ละชนิดที่พบใน dropdown เพื่อให้เข้าใจว่าทำไม lab นี้จึงเลือกใช้ **A record**

| TYPE | ชื่อเต็ม | หน้าที่ | ตัวอย่าง | ใช้ใน lab นี้ |
|---|---|---|---|---|
| **A** | Address | แมปชื่อโดเมน → **IPv4 address** | `stock` → `103.91.190.181` | ✅ ใช้ (หัวใจของ lab) |
| **AAAA** | Quad-A | แมปชื่อโดเมน → **IPv6 address** (เหมือน A แต่เป็น IPv6) | `stock` → `2400:abcd::1` | ใช้ถ้า VPS มี IPv6 |
| **CNAME** | Canonical Name | สร้าง **alias** ชี้ชื่อโดเมนหนึ่งไปยังอีกชื่อ (ไม่ใช่ IP) ห้ามใช้ที่ root/apex domain | `www` → `stock.aibisec.com` | ทางเลือก (ทำ www) |
| **MX** | Mail Exchange | ระบุ **mail server** ที่รับอีเมลของโดเมน มีค่า priority | `aibisec.com` → `mail.aibisec.com` (prio 10) | ไม่ใช้ |
| **TXT** | Text | เก็บ **ข้อความ** อิสระ ใช้ยืนยันตัวตน (SPF, DKIM, domain verification, DNS-01 challenge) | `v=spf1 include:_spf.google.com ~all` | ไม่ใช้ (lab นี้ใช้ HTTP challenge) |
| **NS** | Name Server | ระบุ **authoritative name server** ที่ดูแล DNS ของโดเมน | `aibisec.com` → `ns1.name.com` | ตั้งโดยผู้ให้บริการ |
| **SRV** | Service | ระบุ **host + port** ของ service เฉพาะ (SIP, XMPP, เกม) | `_sip._tcp` → `10 5 5060 sip.example.com` | ไม่ใช้ |
| **ANAME** | Alias (ALIAS) | คล้าย CNAME แต่ **ใช้ที่ root/apex domain ได้** (resolver คืนค่าเป็น A ให้) | `aibisec.com` → `stock.aibisec.com` | ทางเลือก |

**สรุปสำหรับ lab นี้:** เราต้องการให้ชื่อ `stock.aibisec.com` ชี้ตรงไปยัง **IPv4** ของ VPS (`103.91.190.181`) จึงเลือก **A record** เป็นคำตอบที่ถูกต้อง ส่วน CNAME/ANAME เหมาะกับกรณีชี้ชื่อไปยัง "อีกชื่อหนึ่ง" ไม่ใช่ชี้ไปยัง IP โดยตรง

> **Key Concept:** จำง่าย ๆ - **A = ชี้ไป IPv4**, **AAAA = ชี้ไป IPv6**, **CNAME/ANAME = ชี้ไปอีกชื่อ**, **MX = อีเมล**, **TXT = ยืนยันตัวตน**, **NS = ใครดูแล DNS**, **SRV = service + port** เมื่อ deploy เว็บบน VPS ที่มี IP ตายตัว ให้ใช้ A record เสมอ
...
**💬 Prompt ที่ใช้สั่ง Claude Code:**
```
ช่วยอธิบายขั้นตอนการตั้ง DNS A record สำหรับ stock.aibisec.com
...
**✅ Checkpoint ตรวจสอบ:**
- `nslookup stock.aibisec.com` ต้องแสดง IP ของ VPS เราถูกต้อง
- `dig stock.aibisec.com +short` ต้องได้ IP เดียวกัน
...
และรัน certbot --nginx สำหรับ domain stock.aibisec.com
อธิบาย:
...
- เปิด `https://stock.aibisec.com` ใน browser ต้องมี padlock icon
- ตรวจ `sudo systemctl status certbot.timer` ต้อง active
- ทดสอบ `curl -I http://stock.aibisec.com` ต้องได้ `301 Moved Permanently` ไป HTTPS
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/stock.aibisec.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/stock.aibisec.com/privkey.pem;
...
**💬 Prompt ที่ใช้สั่ง Claude Code:**
```
ระบบ StockApp production ไม่ตอบสนอง curl https://stock.aibisec.com ได้ 502 Bad Gateway
...
1. ติดตั้ง nginx
2. สร้างและเปิดใช้ nginx config สำหรับ stock.aibisec.com
3. ออก SSL certificate ด้วย Certbot
4. ทดสอบ https://stock.aibisec.com

domain stock.aibisec.com ชี้ A record มาที่ IP แล้ว
```

**✅ Checkpoint:** `https://stock.aibisec.com` เปิดใน browser ได้ มี padlock, `sudo certbot renew --dry-run` ผ่าน
...
| เกณฑ์ | วิธีตรวจสอบ |
|---|---|
| เว็บเปิดได้ผ่าน HTTPS | เปิด `https://stock.aibisec.com` ใน browser |
...
- ✅ ผูกโดเมน `stock.aibisec.com` และเปิด HTTPS ด้วย Let's Encrypt (Certbot) ฟรี
...
- Production: https://stock.aibisec.com
