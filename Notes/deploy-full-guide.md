...
หมายเหตุ: ชื่อ image ต้องตรงกับที่ workflow ใน repo stock-app-claude build ขึ้น ghcr.io ถ้า workflow ตั้งชื่ออื่น ให้แก้บรรทัด `image:` ให้ตรงกัน
...
ผลลัพธ์ที่ถูกต้อง: แสดงรายการ migration ที่ถูก apply โดยไม่มี error
...
|   docker compose pull        (ดึง image ล่าสุด)                     |
|   docker compose up -d       (รัน version ใหม่)                     |
|   npx prisma migrate deploy  (อัปเดตฐานข้อมูล)                      |
|                                                                    |
...
flowchart TB
    ...
    subgraph GHA["GitHub Actions"]
        J1["Job 1: build-and-push<br>ใช้ GITHUB_TOKEN (อัตโนมัติ)"]
        J2["Job 2: deploy<br>ใช้ (1) VPS_HOST<br>(2) VPS_USER<br>(3) VPS_SSH_PRIVATE_KEY"]
    end

    REPO -->|trigger workflow| J1
    J1 -->|push image| GHCR["ghcr.io<br>stock-app-claude:latest"]
    J1 -->|สำเร็จแล้ว needs| J2
    J2 -->|"SSH เข้า deploy@103.91.190.181<br>ด้วย private key (จับคู่ public key บน server)"| VPS
...
## 11.2 ไฟล์ Workflow .github/workflows/deploy.yml

ใน repo stock-app-claude ต้องมีไฟล์ `.github/workflows/deploy.yml` โครงสร้างดังนี้
...
1. จะเห็น workflow "Deploy to Production" กำลังรัน (จุดสีเหลืองหมุน)
2. Job `build-and-push` เสร็จก่อน (เครื่องหมายถูกเขียว)
3. Job `deploy` รันต่อจนเขียวทั้งหมด
4. เปิด https://stock.aibisec.com ต้องเห็นการเปลี่ยนแปลงที่เรา push ไป

เท่านี้ทุกครั้งที่ push ขึ้น branch main ระบบจะ deploy ให้อัตโนมัติ
...
GitHub (push main) -> GitHub Actions -> build image -> ghcr.io -> SSH deploy -> Server
```
...
- [ ] Container db (healthy) และ app (Up) รันปกติ พร้อม migrate สำเร็จ
- [ ] Nginx reverse proxy ทำงาน เปิดผ่านโดเมนได้
- [ ] HTTPS ทำงาน มี padlock และ `certbot renew --dry-run` ผ่าน
- [ ] Push code ขึ้น main แล้ว GitHub Actions deploy อัตโนมัติสำเร็จ
